...
class MSIR(torch.nn.Module):
    ...
    def __init__(self, args = None):
        ...
        self.beta = torch.tensor([1.30537122, 3.11852393, 0.03206678, 3.24118948, 0.31400672, 0.09410381], requires_grad=False, device= device)
    ...
    def forward(self, t, states):
        ...
        with torch.set_grad_enabled(True):
            ...
            beta_t_pre = self.beta[:, None] * self.contact # I * I
            #print(self.contact.device, self.beta.device)
            ...
            #print(beta_t_pre.device, beta_cos.device)
            #for i in range(B):
            #    beta_t_pre[i,:, :] *= beta_cos[i]
            #beta_t = beta_t_pre#torch.matmul(beta_cos, beta_t_pre)
            ...
            db = zero_tensor
            params = torch.cat((db,db,db, db, db, db, db),1).to(beta_cos.device)
        # reparameterization
...
def ODE_Solver(params, tt, model, args, t_end = None, initial_params =None):
    ...
    #print(non_params.device)
    non_params = non_params.repeat(batch_size,1).to(args.device)
    final_params = torch.cat((use_params,non_params),1)
...
if __name__ == "__main__":
  ...
  init.vals = c(M=c(M.0,rep(0,n.age-1)),S=rep(eq/n.age,n.age),Is=rep(eq/n.age,n.age),Im=rep(eq/n.age,n.age),R=rep(eq/n.age,n.age-1))
    ...
    if reparam :
        ...
        #print(func.t_end)
        #raise ValueError
        #tt = torch.cat((torch.tensor([0.0]), torch.tensor([1.0])))
        tt /= t_end.clone()
        print(tt)
    ...
    params = torch.tensor([[0.43, 7.35,1,2,1/13,1/52, N]], requires_grad=True)
    ...
    #print(params)
    tt = torch.linspace(0, term, steps = term + 1)
    z_samples = torch.transpose(z_samples, 0, 1) # B * t * I
    ...
    #r :  B * 1
    #check this
    #samples = samples.detach().numpy() # B * 118 * 3
    #amples = torch.round(samples)
    print(sample.shape)
    fig, axs = plt.subplots(2,1)
    axs[0].plot(sample[0][0])
    axs[1].plot(sample.sum(dim=3)[0][0])
    plt.show()
    #print(sample)
    for i in range(2, 49):
        torch.manual_seed(819+i)
        sample = nb.sample((1,))
        if not reparam :
            np.save("../data/MSIR_{}".format(i+2), sample[0])
