...
setting_choices = {"MCMC"}

parser.add_argument('--setting', type=str, default="MCMC", choices=setting_choices)
...
if __name__ == "__main__":
    ...
    #genfromtxt(path, delimiter=',', skip_header=True)
    ...
    """
    params = genfromtxt("Sampled_0706.csv", delimiter=',', skip_header=True)
    params[:, [1, 3, 4, 0, 2, 5, 6]] = params[:, [0, 1, 2, 3, 4, 5, 6]]
    print(params.shape)
    params = params[np.linspace(0, params.shape[0] - 1, num=500).astype(int), :]
    ...
    samples_f, age, r, logit = only_forward(param_sample=params, args=args)
    #print(age.shape)
    #print(samples_f.shape)
    samples = (samples_f.detach().cpu().numpy(), age.detach().cpu().numpy(), r, logit)
    samples_age_prop = np.asarray(age)
    age_perc = np.asarray([[42, 30, 28]])
    res = np.mean(np.square((samples_age_prop*100 - age_perc)))
