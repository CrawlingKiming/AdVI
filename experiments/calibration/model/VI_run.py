...
#        if args.log_tb:
    ...
    def model_store(self, model_num, itr, optimizer, scheduler, model):
        now = datetime.now()
        ...
        torch.save({
            ...
            'scheduler': scheduler.state_dict()},
                    os.path.join(self.log_path, '{}_checkpoint_date{}_itr{}_modelnum{}.pt'.format(model_num, dt_string,itr, model_num)))
    ...
    def param_img_store(self, params, model_num, itr):
        ...
        for j in range(6):
            ...
            axs[j // 4 + 1, j % 4].set_xlim([0.0, 3.5])
    ...
    def param_ls_img_store(self, params, model_num, itr, qq = 0):
        ...
        for p_idx in range(len(params)):
            ...
            axs[0, 2].axvline(x=0.027, color="r")
            axs[0, 3].set_title(r'$\nu$')
            axs[0, 3].set_xlim([0.7, 1.2])
            axs[0, 3].axvline(x=0.9, color="r")
            axs[0, 3].hist(temp[:, 3], bins=128)

            axs[1, 1].set_xlim([2,3.5])
            axs[1, 3].set_xlim([2,3.5])
            for j in range(6):
                axs[j // 4 + 1, j % 4].hist(temp[:, j + 4], bins=128)
...
#        if args.log_tb:
    ...
    def forward_img_store(self, samples, truex, groundtruth, model_num, itr):
        ...
        ax.plot(samples_min[:, 1], "--", color="red")
        ax.plot(samples_max[:, 1], "--", color="red")
        ax.plot(truex[0, :, 0], "rx", markersize=2)
        ...
        #ax.plot(truex[0, :, 1], "rx", markersize=2)
        ...
        print(cover_num / model_num*2)
        """
        plt.savefig(os.path.join(self.log_path, "Forwardsamples_model_num{}_interval{}.png".format(model_num, itr)),
                    bbox_inches='tight')
        plt.close(fig=fig)
        plt.clf()
        np.savetxt(os.path.join(self.log_path, "Forward.csv"), samples[:,:,1], delimiter=",")
        #self.writer.add_image('forward_{}'.format(model_num), fig, itr)
    ...
    def param_img_store(self, params, model_num, itr):
        ...
        plt.close(fig=fig)
        plt.clf()
        np.savetxt(os.path.join(self.log_path, "param.csv"), temp, delimiter=",")
        #self.writer.add_image('params_{}'.format(model_num), fig, itr)
...
#        if args.log_tb:
    ...
    def param_ls_img_store(self, params, model_num, itr):
        ...
        for p_idx in range(len(params)):
            ...
            axs[0, 0].hist(temp[:, 0], bins=128, edgecolor='black', color='gray', alpha=0.3)
            axs[0, 0].set_title(r'$\theta_{2}$', size=23)
            #axs[0, 1].set_xlim([0.0 , 5.5])  # 130
            axs[0, 1].hist(temp[:, 1], bins=128, edgecolor='black', color='gray', alpha=0.3)
            axs[0, 1].set_title(r'$\theta_{3}$', size=23)

            axs[0, 2].hist(temp[:, 2], bins=128, edgecolor='black', color='gray', alpha=0.3)
            axs[0, 2].set_title(r'$\theta_{5}$', size=23)
            #axs[1, 1].set_xlim([0.0 , 5.5])  # 0.09
            axs[1, 0].hist(temp[:, 3], bins=128, edgecolor='black', color='gray', alpha=0.3)
            axs[1, 0].set_title(r'$\theta_{6}$', size=23)
            axs[1, 1].set_title(r'$\tau$', size=23)
            #axs[3, 0].set_xlim([0.0, 5.5])  # 0.09
            axs[1, 1].hist(temp[:, 4], bins=128, edgecolor='black', color='gray', alpha=0.3)
            axs[1, 2].set_title(r'$\sigma$', size=23)
...
class WRF_Experiment_Writing():
    ...
    def __init__(self, args,
                 data_id, model_id):
        dir_path = os.path.dirname(os.path.realpath(__file__))
        log_base = os.path.join(dir_path, 'results/WRF/log')
        self.log_base = log_base
        print("Saving log : {}".format(log_base))
        if args.name is None:
            args.name = time.strftime("%Y-%m-%d_%H-%M-%S")
...
#        if args.log_tb:
    ...
    def forward_img_store(self, samples, truex, model_num, itr):
        I_w = samples 
        truex, cov_theta_est, mat_Y_em, mat_Y_mean, mle_est, param_par_rescale, param_par = truex

        truex = truex.detach().cpu().numpy()
        mat_Y_mean = mat_Y_mean.detach().cpu().numpy()
        #print(truex.shape, I_w.shape, mat_Y_mean.shape)
        #raise ValueError
        mat_Y_mean = mat_Y_mean[:, 0]
        I_w = I_w[:, 0, :] + mat_Y_mean[np.newaxis, :] 
        #truex = truex[np.newaxis, :]
        #truex = truex[0]#np.sum(truex, axis=1)
        fig, ax = plt.subplots(1, 1)
        ...
        plt.savefig(os.path.join(self.log_path, "Forwardsamples_model_num{}_interval{}.png".format(model_num, itr)),
                    bbox_inches='tight')
        plt.close(fig=fig)
        np.save(os.path.join(self.log_path, "Forwardsamples"), I_w)
    ...
    def param_ls_img_store(self, params, model_num, itr):
        ...
        for p_idx in range(len(params)):
            ...
            temp = temp[0]
            np.savetxt(os.path.join(self.log_path, "temp.csv"), temp, delimiter=",")
            ...
            axs[0, 2].hist(temp[:, 2], bins=128, edgecolor='black', color='gray', alpha=0.3)
            axs[0, 2].set_title(r'$\theta_{3}$', size=23)
            #axs[0, 2].set_xlim([0.4 , 0.7])  # 0.09
            axs[1, 0].hist(temp[:, 3], bins=128, edgecolor='black', color='gray', alpha=0.3)
            axs[1, 0].set_title(r'$\theta_{4}$', size=23)
            axs[1, 1].set_title(r'$\theta_{5}$', size=23)
            #axs[3, 0].set_xlim([0.0, 5.5])  # 0.09
            axs[1, 1].hist(temp[:, 4], bins=128, edgecolor='black', color='gray', alpha=0.3)
            axs[1, 2].set_title("range_disc", size=23)
            #axs[1, 2].set_xlim([0.0,  100])  # 0.09
            axs[1, 2].hist(temp[:, 5], bins=128, edgecolor='black', color='gray', alpha=0.3)
            axs[2, 0].set_title("sill_disc", size=23)
            #axs[3, 1].set_xlim([0.0, 5.5])  # 0.09
            axs[2, 0].hist(temp[:, 6], bins=128, edgecolor='black', color='gray', alpha=0.3)
            axs[2, 1].set_title("nugget_disc", size=23)
            #axs[3, 1].set_xlim([0.0, 5.5])  # 0.09
            axs[2, 1].hist(temp[:, 7], bins=128, edgecolor='black', color='gray', alpha=0.3)
            axs[2, 2].set_title("sill_y", size=23)
            #axs[3, 1].set_xlim([0.0, 5.5])  # 0.09
            axs[2, 2].hist(temp[:, 8], bins=128, edgecolor='black', color='gray', alpha=0.3)
