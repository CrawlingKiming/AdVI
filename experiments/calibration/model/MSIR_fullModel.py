...
if __name__ == "__main__":
  ...
  init.vals = c(M=c(M.0,rep(0,n.age-1)),S=rep(eq/n.age,n.age),Is=rep(eq/n.age,n.age),Im=rep(eq/n.age,n.age),R=rep(eq/n.age,n.age-1))
    ...
    beta = torch.tensor([[1.30537122, 3.11852393, 0.03206678, 3.24118948, 0.31400672, 0.09410381]])

    b = b.repeat(B, 1)
    r = r.repeat(B, 1)
    rho = rho.repeat(B, 1)
    beta = beta.repeat(B,1)
    print(beta.shape)
    ...
    #print(cases_prob)
    #print(cases_prob.shape)
    nb = torch.distributions.negative_binomial.NegativeBinomial(total_count=new_r, probs=cases_prob)


    cases_fit = cases_fit.detach().cpu().numpy()
    np.save("../data/MSIR_lambda", lambd[[0]])
    np.save("../data/ground_truth66", samples[[0]])
    for i in range(50):
        random_seed = i
        torch.manual_seed(random_seed)
        real_sample = nb.sample((1,))

        real_sample = real_sample.detach().cpu().numpy()
        np.save("../data/MSIR_{}.npy".format(random_seed), real_sample[0][[0]])

    print(real_sample[0])
    #np.save("../data/MSIR_67.npy", real_sample[0][[0]])
    #print(real_sample[0][0].shape)
    plt.plot(np.sum(real_sample[0][0], axis=1))
    plt.plot(np.sum(cases_fit[0], axis= 1))
    plt.show()
    #plt.plot(torch.sum(cases_fit[0], =1))
    #plt.plot(np.sum(real_sample[0], axis=1))
    #plt.show()
    print(lambd[[0]].shape, samples[0].shape)
